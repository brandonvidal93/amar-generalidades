    ...
    if (actualIndex !== 0) {
      return(
        <div className = 'buttonPannel'>
          ...
          <button
            className = { 'buttonNav ' + (actualIndex === (limitNavigation + 1) || Object.values(data)[actualIndex].endCourse === true ? 'disabled ': '') + (actualIndex === 1 || actualIndex === 2 || actualIndex === 3 || actualIndex === 17 || actualIndex === 20 ? 'disabled ': '') }
            id = 'btnNavRight'
            onClick = { this.navigationCourse }>
            <FontAwesomeIcon
              className = 'iconButton'

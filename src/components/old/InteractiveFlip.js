          ...
          <div className = 'interactiveFlip'>
            {
              dataPage.items.map((item, i) => {
                return(
                  <div key = { item.id }>
                    <div className = 'buttonFlip d-Flex d-C j-C aI-C' id = { item.id } onClick = { this.flip }>
                      <img alt = 'Item' className = 'mB-1' src = { item.img }/>
                      <h3 className = 'tCenter mB-1' dangerouslySetInnerHTML = {{ __html: item.title }}></h3>
                    </div>

                    <div className = 'infoFlip d-Flex d-C j-C aI-C dNone' id = { 'info-' + item.id }>
                      <h3 className = 'tCenter mB-1' dangerouslySetInnerHTML = {{ __html: item.title }}></h3>
